...
OUTPUT_DIR = "/home/joberant/NLP_2425b/maozhaim/NLP/gpt-neo/fine-tuning-results"
MODEL_SAVE_PATH = "/home/joberant/NLP_2425b/maozhaim/NLP/gpt-neo/gpt-neo-fine-tuned-model"
LORA_DIR  = "/home/joberant/NLP_2425b/maozhaim/NLP/gpt-neo/lora"
MAX_LEN = 300
EPOCHS = 3
LEARNING_RATE = 6e-5 # a bit high for LoRA
...
if tokenizer.pad_token is None: # GPT-Neo/GPT-2 tokenizers often don't define pad_token; use eos_token instead
    tokenizer.pad_token = tokenizer.eos_token

# Base model (float32, no fp16)
...
# Apply LoRA adapters to attention projections (names typical for GPT-Neo)
lora_cfg = LoraConfig(
    r=8,
    lora_alpha=16,
    target_modules=["q_proj", "k_proj", "v_proj", "out_proj"],  # GPT-Neo attention proj layers
    lora_dropout=0.05,
    bias="none",
    task_type="SEQ_CLS"
)
...
model.gradient_checkpointing_enable() # save activations memory
model.config.use_cache = False # must be False when gradient checkpointing is on
model.to(device)
print()
...
fine_tuning_args = TrainingArguments(
    ...
    per_device_train_batch_size=1, # tiny batch to fit 12GB VRAM
    ...
    save_total_limit=2,
    push_to_hub=False,
    gradient_checkpointing=True, # also set here for Trainer
    dataloader_pin_memory=False # can help a bit on older GPUs
)
print()
...
print("--- Save model ---")
# 1) Save LoRA adapter (compact, can resume LoRA training later)
fine_tuning_trainer.model.save_pretrained(LORA_DIR)
tokenizer.save_pretrained(LORA_DIR)

# 2) Merge LoRA into base weights and save a standard HF model for easy inference/pipeline
...
fine_tuning_results = fine_tuning_pipe(texts, batch_size=4)
